...
class Counter8:
    def __init__(self,game):
        self.game=game
        self.count = game.Decimal(self.game.Save.counters_data[7][0][0],self.game.Save.counters_data[7][0][1],game)
        self.multi = game.Decimal(self.game.Save.counters_data[7][1][0],self.game.Save.counters_data[7][1][1],game)
        self.cost = game.Decimal(self.game.Save.counters_data[7][2][0],self.game.Save.counters_data[7][2][1],game)
        self.cost_up = game.Decimal(1,15,game)
        self.cost_up_inf=8
        self.produce_base=1
    ...
    def place(self):
        ...
        self.text=self.game.main_canvas.create_text(self.game.geometry[0]-(self.game.geometry[0]-85),790,anchor='w',text=self.notation('total'),fill='#c22f40',font=('bahnschrift',24))
        self.text_multi = self.game.main_canvas.create_text(self.game.geometry[0] - (self.game.geometry[0] - 83), 815,
    ...
    def produce(self):
        if self.game.Counter_7.first:
            mult = self.game.Decimal(self.multi.num, self.multi.e, self.game)
            self.produce_count=(mult*self.produce_base*self.count*self.game.Tickspeed.get_time()*self.game.Achievements.achieve_mult('8 Counter')*self.game.Eternity.time_speed*self.game.CB.multi_list[7]
    ...
    def reset(self):
        ...
        self.cost = self.game.Decimal(1, 22, self.game)
