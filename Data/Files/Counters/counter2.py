...
class Counter2:
    def __init__(self,game):
        self.game=game
        self.count = game.Decimal(self.game.Save.counters_data[1][0][0],self.game.Save.counters_data[1][0][1],game)
        self.multi = game.Decimal(self.game.Save.counters_data[1][1][0],self.game.Save.counters_data[1][1][1],game)
        self.cost = game.Decimal(self.game.Save.counters_data[1][2][0],self.game.Save.counters_data[1][2][1],game)
        self.cost_up=game.Decimal(1,3,game)
        self.cost_up_inf=8
        self.produce_base=1
    ...
    def place(self):
        ...
        self.text=self.game.main_canvas.create_text(self.game.geometry[0]-(self.game.geometry[0]-85),310,anchor='w',text=self.notation('total'),fill='#c22f40',font=('bahnschrift',24))
        self.text_multi = self.game.main_canvas.create_text(self.game.geometry[0] - (self.game.geometry[0] - 83), 335,
    ...
    def produce(self):
        if self.game.Counter_1.first:
            ...
            self.produce_count=(mult*self.produce_base*self.count*self.game.Tickspeed.get_time()*self.game.Eternity.time_speed*self.game.Achievements.achieve_mult('2 Counter')*self.game.CB.multi_list[1]
    ...
    def notation(self,type):
        if type=='total':
            ...
            mult = self.game.Decimal(self.multi.num, self.multi.e, self.game)
            mult * self.game.CB.multi_list[1]*self.game.Achievements.achieve_mult('2 Counter') * self.game.Infinity.get_boost('2 counter')
            return mult.get(2,4)
        elif type=='color_x':
            mult = self.game.Decimal(self.multi.num, self.multi.e, self.game)
            mult * self.game.CB.multi_list[1]* self.game.Achievements.achieve_mult('2 Counter') * self.game.Infinity.get_boost('2 counter')
    ...
    def reset(self):
        ...
        self.cost = self.game.Decimal(1, 2, self.game)
        self.cost_up = self.game.Decimal(1, 3, self.game)
